...
Route.post('/applications/:name/configuration', async ({ request, params, response }) => {
  const { buildPack, port, installCommand, buildCommand, startCommand } = request.body()
  const applicationFound = await Application.findByOrFail('name', params.name)
  await applicationFound.merge({ buildPack, port, installCommand, buildCommand, startCommand }).save()
